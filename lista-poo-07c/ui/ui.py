...
class UI:
    ...
    def produto_atualizar() -> None:
        ...
        est = input("Novo estoque: ")
        preco = None if not preco else float(preco)
        est = None if not est else int(est)
        print("Categorias disponíveis:")
        categorias = View.categoria_listar()
        for categoria in categorias:
            print("\t", categoria)
        id_c = input("ID da Categoria: ")
        id_c = None if not id_c else int(id_c)
        View.produto_atualizar(id, desc, preco, est, id_c)
